...
cat ${ref}-$spe.delta.filt.coords.Z-perc | awk 'BEGIN{while(getline < "'${ref}'-'$spe'.delta.filt.coords.A-perc"){a[$1]=$3}}{print $0"\t"$3-a[$1]}' > ${ref}-$spe.delta.filt.coords.ZsubA-perc
